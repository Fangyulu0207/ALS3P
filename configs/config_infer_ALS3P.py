"""CLI configuration for selected-layer similarity-map visualization."""
...
parser = argparse.ArgumentParser(description="Export ALS3P chosen layers")
...
parser.add_argument("--name", type=str, default="ALS3P_layers")
parser.add_argument(
...
parser.add_argument(
    "--visualization_root",
    type=str,
    default="my_visualization_triple-selector-v3-layers",
)
parser.add_argument(
    "--visualize_splits",
    type=str,
    default="test_seen,test_unseen,test_null",
    help="Comma-separated choices: test_seen,test_unseen,test_null.",
)
parser.add_argument(
    "--max_visualizations",
    type=int,
    default=-1,
    help="Maximum dataset samples exported per split; -1 exports all samples.",
)
parser.add_argument(
    "--heatmap_alpha",
    type=float,
    default=0.45,
    help="Opacity of the similarity heatmap over the original frame.",
)
...
if args.layer_strategy != "selector":
    parser.error("this visualization requires --layer_strategy selector")
if args.eval_layer_strategy != "argmax":
    parser.error("this visualization requires --eval_layer_strategy argmax")
if args.map_loss_weight != 0.0:
    parser.error("this no-map-loss checkpoint requires --map_loss_weight 0")
if args.selector_loss_weight <= 0.0:
