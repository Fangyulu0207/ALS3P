...
def merge_tokens(token_dict, idx_cluster, cluster_num, token_weight=None):
    ...
    out_dict['idx_token'] = idx_token_new  #idx_token记录了每个原始token属于哪个cluster
...
class CTM(nn.Module):
    ...
    def forward(self, token_dict, sample_ratio=None):
        ...
        down_dict = merge_tokens(token_dict, idx_cluster, cluster_num, token_weight)  #把同一簇的 token 加权平均 掉

        # ── 新增：把 idx_cluster 存入 down_dict，供 AML 使用 ──
        # idx_cluster: (B, N_original) 每个原始token属于哪个cluster
        down_dict['idx_cluster'] = idx_cluster  # (B, N_original)
        # ────────────────────────────────────────────────────────
